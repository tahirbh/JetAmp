  ...
  return (
    <div className={`grid ${isCinemaMode ? 'grid-rows-[auto_1fr]' : 'grid-rows-[auto_1fr_auto]'} h-full w-full bg-gradient-to-b from-transparent to-[var(--bg-dark)]/40 relative overflow-hidden transition-all duration-700`}>
      ...
      <div className={`w-full flex items-center justify-center px-4 mb-2 min-h-0 relative ${!isCinemaMode ? '-mt-2' : ''}`}>
        ...
        <div className={`relative rotating-cd-container w-full h-full flex items-center justify-center ${!isCinemaMode ? 'max-h-[340px] aspect-square mt-6' : 'max-h-none mt-0'}`}>
          
          {/* Layer 1: Content (CD or Video) */}
          <div className={`z-10 w-full h-full flex items-center justify-center relative overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.8)] ${!isCinemaMode ? 'rounded-full border-4 border-[var(--metal-dark)]' : 'rounded-xl border border-white/10'}`}>
            {isYouTube ? (
          ...
          {/* Layer 2: Dash Overlays (HIDDEN IN CINEMA) */}
          {!isCinemaMode && (
      ...
      {/* 3. BOTTOM ROW: Transport Controls (HIDDEN IN CINEMA) */}
      {!isCinemaMode && (
        <div className="flex flex-col bg-transparent backdrop-blur-none -mx-4 px-4 pb-2 z-20 relative -mt-8">
          ...
          <div className="flex items-center justify-center gap-1 sm:gap-4 md:gap-6 flex-shrink-0 scale-[0.55] sm:scale-[0.75] 2xl:scale-100 origin-center transition-transform duration-300 py-0 pb-1">
            ...
            >
               <div className="text-[10px] font-black">EQ</div>
