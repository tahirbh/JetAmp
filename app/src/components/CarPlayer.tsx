...
export function CarPlayer({
  ...
  const isYouTube = currentTrack?.source?.toLowerCase() === 'youtube' || 
                 currentTrack?.url?.toLowerCase().includes('youtube.com') || 
                 currentTrack?.url?.toLowerCase().includes('youtu.be');
  ...
  return (
    <div className="grid grid-rows-[auto_1fr_auto] h-full w-full bg-gradient-to-b from-transparent to-[var(--bg-dark)]/40 relative overflow-hidden">
      ...
      <div className="w-full flex items-center justify-center px-4 -mt-2 mb-2 min-h-0 relative">
        ...
        <div className={`relative rotating-cd-container w-full h-full flex items-center justify-center max-w-[340px] max-h-[340px] aspect-square mt-6 mx-auto ${isYouTube ? 'landscape-full-height' : ''}`}>
          
          {/* Layer 1: Rotating CD or YouTube Player */}
          <div className={`z-10 w-full h-full flex flex-col items-center justify-center ${isYouTube ? 'landscape-full-height' : ''}`}>
            {isYouTube && (
            ...
            <div className="flex-1 w-full flex items-center justify-center">
              {currentTrack?.source === 'youtube' ? (
                <YouTubePlayer 
                  ...
                  onEnded={onNext}
                  showNativeControls={true} // Enabled globally for now, or could be dynamic
                />
              ) : (
                <RotatingCD
